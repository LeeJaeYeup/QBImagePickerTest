...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.imagePickerController.selectedAssets containsObject:asset]) {
        [self customSelectAssetNumberToCell:collectionView IndexPath:indexPath addCell:cell];
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self customSelectAssetNumberToCell:collectionView IndexPath:indexPath addCell:nil];
...
- (void)customSelectAssetNumberToCell:(UICollectionView *)collectionView IndexPath:(NSIndexPath *)indexPath addCell:(QBAssetCell *)cell
{
    PHAsset *asset = self.fetchResult[indexPath.item];
    int selectNum = 0;
    for (int i =0 ; i < self.imagePickerController.selectedAssets.count ; i ++) {
        
        if ([[self.imagePickerController.selectedAssets objectAtIndex:i] isEqual:asset]) {
            selectNum = i;
            break;
        }else {
            selectNum ++;
    ...
    NSLog(@"selectAssetNum : %d",selectNum);
    QBAssetCell *selectCell;
    if (!cell) {
        selectCell = (QBAssetCell *)[collectionView cellForItemAtIndexPath:indexPath];
    }else {
        selectCell = cell;
    }
    selectCell.selectAssetNum = selectNum;
}
